...
def predict_next_5_candles_ema21(df, period=21, slope_lookback=5):
    """
    Predict direction for the next 5 candles based on EMA(21).
    Uses:
      - Price relative to EMA (above = bullish, below = bearish)
      - EMA slope over last `slope_lookback` periods (adjusts confidence)
    ...
    latest_ema = df['ema_21'].iloc[-1]

    # Compute EMA slope over last slope_lookback periods
    if len(df) >= slope_lookback:
        ema_prev = df['ema_21'].iloc[-slope_lookback]
...
def main():
    ...
    if len(df) < 21:
        print(f"Warning: only {len(df)} candles provided. EMA(21) may be unreliable.")

    try:
        prediction_meta = predict_next_5_candles_ema21(df)
    except Exception as e:
        print(f"Error during prediction: {e}")
        return

    output_file = 'EMA21_prediction_output.json'
